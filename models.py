...
Model module for creation and translation
of NEOs and CAs.
"""
...
class NearEarthObject:
    ...
    def __str__(self):
        ...
        return f"NEO: {self.fullname} with a diameter of "\
               f"{self.diameter:.2f} {haz} potentially hazardous"
    ...
    def serialize(self):
        """Return a dict to be serialized into csv or json."""
        ret = {}
...
class CloseApproach:
    ...
    the NEO's close approach to Earth, such as the date
    and time (in UTC) of closest approach, the nominal
    ...
    def time_str(self):
        ...
        `datetime` object. While a `datetime` object
        has a string representation, the default representation
        includes seconds - significant figures that don't exist
        in our input data set.
        The `datetime_to_str` method converts a `datetime`
        object to a formatted string that can be used in
        human-readable representations and in serialization to
        CSV and JSON files.
        """
    ...
    def __str__(self):
        """Return `str(self)`."""
        return f"On {self.time_str}, {self.neo.fullname} passes "\
               f"Earth from {self.distance:.2f}au at {self.velocity:.2f}km/h"
